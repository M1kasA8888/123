...
with tab1:
    ...
    with col_right:
        ...
        if st.session_state.temp_obs:
            st.markdown("### 🆕 新建障碍物")
            name = st.text_input("名称", st.session_state.temp_name)
            st.session_state.temp_name = name if name else "障碍物"
            col1, col2 = st.columns(2)
            with col1:
                min_h = st.number_input("最低(m)", st.session_state.temp_h[0], 0, 200, 5)
            with col2:
                max_h = st.number_input("最高(m)", st.session_state.temp_h[1], min_h+1, 300, 5)
            st.session_state.temp_h = [min_h, max_h]
            st.progress(min(1.0, max_h/300))
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ 保存", type="primary"):
                    st.session_state.obstacles.append(Obstacle(st.session_state.temp_obs, min_h, max_h, name))
                    st.session_state.temp_obs = None
                    st.rerun()
            with col2:
                if st.button("🗑️ 取消"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            la = st.number_input("纬度", st.session_state.point_a[0], format="%.6f")
        with col2:
            lo = st.number_input("经度", st.session_state.point_a[1], format="%.6f")
        if st.button("📍 设置A点"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            lb = st.number_input("纬度", st.session_state.point_b[0], format="%.6f", key="lb")
        with col2:
            lob = st.number_input("经度", st.session_state.point_b[1], format="%.6f", key="lob")
        if st.button("🏁 设置B点"):
            ...
            if not hit:
                ...
                if left_dist <= right_dist:
                    plans.append({
                        ...
                        'dist': left_dist,
                        'color': 'gold',
                        'desc': f'最短路径，节省{abs(left_dist-right_dist):.0f}m'
                    })
                else:
                    plans.append({
                        'name': '⭐ 最佳航线',
        ...
        if st.session_state.route_plans:
            ...
            for i, p in enumerate(st.session_state.route_plans):
                ...
                if st.session_state.selected_plan and st.session_state.selected_plan['name'] == p['name']:
                    ...
                    if st.button(f"选择", key=f"sel_{i}"):
            ...
            if st.session_state.selected_plan:
                p = st.session_state.selected_plan
                st.info(f"**当前: {p['name']}** | {p['dist']:.0f}m | {p['dist']/15:.0f}s")
...
with tab2:
    ...
    with col1:
        ...
        if not st.session_state.heartbeat_running:
            if st.button("▶️ 启动心跳模拟", type="primary"):
