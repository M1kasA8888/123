    ...
    def plan_route(self, start: List[float], end: List[float]) -> Optional[Dict]:
        ...
        for offset_lat in offsets:
            for offset_lon in offsets:
                ...
                if self.is_point_safe_3d(mid_point):
...
if st.session_state.page == "🗺️ 航线规划":
    st.title("🛰️ 航线规划")
    st.markdown("设置起降点、3D障碍区（含高度），规划安全航线 — **南京科技职业学院**")
    st.markdown("---")
    ...
    with col_right:
        ...
        for i, obs in enumerate(st.session_state.obstacles):
            if obs.min_height <= flight_altitude <= obs.max_height:
                ...
                st.success(f"✅ 障碍物「{obs.name}」高度 {obs.min_height}-{obs.max_height}m 安全")
                ...
                if st.session_state.point_a and st.session_state.point_b:
                    ...
                    if flight_plan:
                        ...
                        if flight_plan['is_safe']:
                            st.success(f"✅ 航线规划成功！{flight_plan['path_type']}")
                        else:
                            st.warning(f"⚠️ {flight_plan.get('warning', '航线需要调整')}")
                        st.rerun()
                    else:
                        st.error("❌ 无法规划安全航线")
                else:
